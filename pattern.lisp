(defpackage "PATTERN-TYPE"
  (:use "CL")
  (:local-nicknames ("C" "CONDITION"))
  (:shadow "TYPE" "FUNCTION")
  (:export "*"
           "TYPE" "FUNCTION" "BOX" "BASE" "VAR" "SPREAD"
           "TYPE-P" "FUNCTION-P" "BOX-P" "BASE-P" "VAR-P" "SPREAD-P"
           "SPLIT" "PARSE"))

(in-package "PATTERN-TYPE")
...
;; single-value type is every pattern type except for spread.
(defstruct (single-value-type
             (:include type)
             (:constructor nil)))

(defstruct (function
             (:include single-value-type)
...
(defstruct (box
  ...
  (contents nil :type single-value-type :read-only t))
...
(defstruct (spread
             (:include type)
             (:constructor spread (flow)))
  (flow nil :type (and cons (satisfies list-of-symbol)) :read-only t))

(defmethod split ((pattern spread))
  (list (spread-flow pattern)))

(defmethod build ((tag (eql 'spread)) arguments)
